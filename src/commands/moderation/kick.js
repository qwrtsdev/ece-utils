...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (!target.kickable) {
                return await interaction.reply({
                    content: '🚫 ไม่สามารถนำผู้ใช้นี้ออกได้ (บทบาทสูงกว่าหรือเป็นเจ้าของเซิร์ฟเวอร์)',
                    flags: MessageFlags.Ephemeral
                });
            }
            ...
            try {
                ...
                if (confirmation.customId === 'confirm_kick') {
                    const processingComponents = [ 
                        new ContainerBuilder()
                            .addTextDisplayComponents(
                                new TextDisplayBuilder().setContent(`### **⌛ กำลังดำเนินการเตะ <@${target.user.id}>**\n-# อาจใช้เวลาดำเนินการ กรุณารอสักครู่..`),
                            )
                    ]
                    ...
                    try {
                        const dmComponents = [
                            new ContainerBuilder()
                                .addTextDisplayComponents(
                                    new TextDisplayBuilder().setContent("# **🚪 คุณถูกนำออกจากเซิร์ฟเวอร์**"),
                                )
                                .addTextDisplayComponents(
                    ...
                    try {
                        ...
                        try {
                            ...
                            const notiComponents = [
                                new ContainerBuilder()
                                    .addTextDisplayComponents(
                                        new TextDisplayBuilder().setContent("# **🚪 ผู้ใช้ถูกนำออกจากเซิร์ฟเวอร์**"),
                                    )
                                    .addTextDisplayComponents(
